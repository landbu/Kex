...
def fastLin(weights, biases, originalClass, targetClass, input, p, epsilon_0, tolerance = 1e-6, maxItersExponential = 20, maxItersBinary = 40):
    m = len(weights)
    ...
    while numIters < maxItersExponential:
        print(f"Exponential search iteration: {numIters}")
        ...
        if lowerBounds[m] > 0:
            ...
            print("Stopping exponential search: found epsilon that is not a certified lower bound")
            break
    ...
    while numIters < maxItersBinary:
        print(f"Binary search iteration: {numIters}, current certified lower bound: {epsilonLow}")
...
if __name__ == "__main__":
    model = LeNet()
    filename = "leNet"
    model.load_state_dict(state_dict = torch.load(f"models/{filename}.pth"))

    parameters = extractParams(model, inputShape = (1, 28, 28))
    ...
    image, label = testingSet[0]
    input = image.view(-1, 1).numpy()

    targetLabel = 0

    certifiedEpsilon = fastLin(weights, biases, label, targetLabel, input, 2, 1.0)
    print(f"Final certified lower bound: {certifiedEpsilon}")
