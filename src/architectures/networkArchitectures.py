...
# "100neurons", for all networks with 100 neurons (feasible for big-M)
...
##### Constant number of hidden neurons  (100) #####

# 28 x 28 -> 100 -> 10
@registerNetwork("dense", "constantWidth", "100neurons")
class Dense1x100(nn.Module):
    def __init__(self):
        super(Dense1x100, self).__init__()
        self.dense1 = nn.Linear(in_features = 1 * 28 * 28, out_features = 100)
        self.relu1 = nn.ReLU()
        self.dense2 = nn.Linear(in_features = 100, out_features = 10)
...
# 28 x 28 -> 25 -> 25 -> 25 -> 25 -> 10
@registerNetwork("dense", "constantWidth", "100neurons")
class Dense4x25(nn.Module):
    def __init__(self):
        super(Dense4x25, self).__init__()
        self.dense1 = nn.Linear(in_features = 1 * 28 * 28, out_features = 25)
        self.relu1 = nn.ReLU()
        self.dense2 = nn.Linear(in_features = 25, out_features = 25)
        self.relu2 = nn.ReLU()
        self.dense3 = nn.Linear(in_features = 25, out_features = 25)
        self.relu3 = nn.ReLU()
        self.dense4 = nn.Linear(in_features = 25, out_features = 25)
        self.relu4 = nn.ReLU()
        self.dense5 = nn.Linear(in_features = 25, out_features = 10)
...
@registerNetwork("dense", "constantWidth", "100neurons", "20width")
...
# 28 x 28 -> 10 -> 10 -> 10 -> 10 -> 10 -> 10 -> 10 -> 10 -> 10 -> 10 -> 10
@registerNetwork("dense", "constantWidth", "100neurons")
class Dense10x10(nn.Module):
    def __init__(self):
        super(Dense10x10, self).__init__()
        ...
        self.relu7 = nn.ReLU()
        self.dense8 = nn.Linear(in_features = 10, out_features = 10)
        self.relu8 = nn.ReLU()
        self.dense9 = nn.Linear(in_features = 10, out_features = 10)
        self.relu9 = nn.ReLU()
        self.dense10 = nn.Linear(in_features = 10, out_features = 10)
        self.relu10 = nn.ReLU()
        self.dense11 = nn.Linear(in_features = 10, out_features = 10)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.relu7(self.dense7(x))
        x = self.relu8(self.dense8(x))
        x = self.relu9(self.dense9(x))
        x = self.relu10(self.dense10(x))
        x = self.dense11(x)
        return x
