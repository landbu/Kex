...
class ModelBridge():
    def __init__(self,torch_model, gurobi_model=None):
        ...
        self.layers = []
        self.gurobi_model.setParam("TimeLimit", 3600)  # seconds
    ...
    def load_layers_from_pytorch_model(self, L0=0, U0=1, input_shape=None, create_input_layer=True):
            ...
            Only handles object initialization and shape inference.
            """
            self.layers = [] # Reset layers list if re-loading
            
            # 1. Handle Input Shape Inference
            if input_shape is None:
                # Look through children until we find a layer with dimensions
                for m in self.torch_model.children():
                    if isinstance(m, nn.Linear):
                        ...
                        continue # Keep looking for the Linear layer after Flatten
            
            # Initialize the input layer
            if create_input_layer: self.add_input_layer(input_shape, L0, U0)
            current_shape = input_shape

            # 2. Iterate through children of self.torch_model
            # Stop before Softmax/LogSoftmax as they aren't linearizable/MILP friendly
            layers_to_process = [m for m in self.torch_model.children() 
                                if not isinstance(m, (nn.Softmax, nn.LogSoftmax))]

            for m in layers_to_process:
                if isinstance(m, nn.Linear):
                    weights = m.weight.detach().cpu().numpy()
                    bias = m.bias.detach().cpu().numpy()
                    
                    new_layer = Linear(self, weights, bias)
                    # Ensure the object knows which logic to use during the Gurobi setup phase
                    #new_layer.setup_gurobi_forward_logic = new_layer.setup_gurobi_forward_logic_ia
                    ...
                    bias = m.bias.detach().cpu().numpy()
                    stride = m.stride # This is typically a tuple (s_h, s_w)
                    
                    new_layer = Convolution(self, kernel, bias, stride, bound_tightening="ia")
                    self.add_layer(new_layer)
                    
                    # Shape Inference: H_out = floor((H_in - K) / S) + 1
                    ...
                    # Maps PyTorch AvgPool2d to your MeanPooling class
                    ...
                    self.add_layer(Reshape(self, (int(flat_size),)))
                    current_shape = (int(flat_size),)

                else:
                    raise NotImplementedError(f"Layer type {type(m)} not implemented.")

            # 3. Append the final OutputLayer 
            # This will link self.output_vars to the last processed layer's output
